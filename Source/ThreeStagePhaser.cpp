...
#include "ThreeStagePhaser.h"

ThreeStagePhaser::ThreeStagePhaser() {
    numFilters = 3;
    apf.resize(numFilters);
    apfOutputs.resize(numFilters);
    apfMinFreq.resize(numFilters);
    apfMaxFreq.resize(numFilters);
    apfMinFreq = {50, 350, 650};
    apfMaxFreq = {3500, 10000, 20000};
}

float ThreeStagePhaser::processSample(float inputSample) {
    ...
    float gamma1 = apf[2].getG_value();
    float gamma2 = apf[1].getG_value() * gamma1;
    float gamma3 = apf[0].getG_value() * gamma2;
    
    float K = mFeedback / 1000;
    float alpha0 = 1.f / (1.f + K * gamma3);
                   
    float Sn = gamma3 * apf[0].getS_Value() +
               gamma2 * apf[1].getS_Value() +
               gamma1 * apf[2].getS_Value();
